...
function _ajax(method, url, data, callback) {
    $.ajax({
        ...
        console.log(jqXHR);
        return callback && callback({error: 'HTTP ' + jqXHR.status, message: jqXHR.responseText + ' ( http: ' + jqXHR.status + ', ' + jqXHR.statusText + ' )'});
    });
}
...
$(function() {
    if (location.pathname === '/' || location.pathname === '/weekly') {
        $('li[data-url=index]').addClass('uk-active');
    }
    if (location.pathname === '/history') {
        $('li[data-url=history]').addClass('uk-active');
    }
    if (location.pathname === '/manage') {
        $('li[data-url=manage]').addClass('uk-active');
    }
});
